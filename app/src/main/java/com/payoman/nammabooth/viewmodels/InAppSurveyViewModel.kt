...
class InAppSurveyViewModel @Inject constructor(
    ...
    suspend fun sendVoterStatus(sendSurveyStatusRequest: SendSurveyStatusRequest) {
        inAppSurveyRepository.sendVoterStatus(sendSurveyStatusRequest)
    }

    suspend fun setColor(voterId: String, color: String) {
        inAppSurveyRepository.saveSelectedColor(voterId, color)
    }
}